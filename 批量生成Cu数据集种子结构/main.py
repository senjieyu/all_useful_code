...
def print_job_info(struct_type, mat_conf, tasks, dirs, v1, v2, info):
    ...
    def est_files(t_name, t_cfg):
        if not t_cfg['enable']: return 0, "DISABLED"
        base = 0
        if t_name == "1d_main":
            base = t_cfg['n_points']
        elif t_name == "1d_sec":
...
if __name__ == "__main__":
    ...
    # --- 1. 用户配置 ---
    STRUCTURE_TYPE = "FCC"  # 尝试改成 "BCC" 或 "HCP" 看看标签变化
    ...
    # --- 2. 路径初始化 ---
    ROOT_DIR = os.getcwd()
    BASIC_MODEL_DIR = os.path.join(ROOT_DIR, "Basic_model")
    BATCH_OUT_DIR = os.path.join(ROOT_DIR, "test_batch_file")
    BATCH_RELAX_DIR = os.path.join(BATCH_OUT_DIR, "to_relax")

    for d in [BASIC_MODEL_DIR, BATCH_OUT_DIR, BATCH_RELAX_DIR]:
        os.makedirs(d, exist_ok=True)

    dirs = {'base_dir': BASIC_MODEL_DIR, 'out': BATCH_OUT_DIR, 'relax': BATCH_RELAX_DIR}
    for key in TASKS:
    ...
    if STRUCTURE_TYPE == "FCC":
        base_atoms, v_main, v_sec, info = builder.build_fcc(
            MAT_CONFIG['element'], MAT_CONFIG['lattice_a'], MAT_CONFIG['size'], vacuum=MAT_CONFIG['vacuum'])
        ...
        base_atoms, v_main, v_sec, info = builder.build_hcp(
            MAT_CONFIG['element'], MAT_CONFIG['lattice_a'], c_val, MAT_CONFIG['size'], vacuum=MAT_CONFIG['vacuum'])
    else:
        raise ValueError(f"Unknown Structure Type: {STRUCTURE_TYPE}")

    mc.AtomUtils.write_vasp_file(base_atoms, f"Base_{STRUCTURE_TYPE}_{MAT_CONFIG['element']}", BASIC_MODEL_DIR)
    ...
    if TASKS['2d_surf']['enable']:
        # 2D 表面需要原始基矢量，我们重新获取一下
        raw_v1 = base_atoms.get_cell()[0] / MAT_CONFIG['size'][0]
        raw_v2 = base_atoms.get_cell()[1] / MAT_CONFIG['size'][1]
        total_gen += batch.generate_2d_surface(base_atoms, raw_v1, raw_v2, TASKS['2d_surf'], dirs)
