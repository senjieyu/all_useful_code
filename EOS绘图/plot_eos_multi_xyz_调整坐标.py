...
新增：
- 默认对每条曲线做能量零点对齐：ΔE = E - Emin（Emin 为该方法数据点最小值）
- 用 --no-emin-align 可关闭（保留绝对能量）
- 若启用对齐，表格会额外输出 Emin_raw(eV/atom)，便于还原绝对能量

单位：
- V0：Å^3/atom
- E0：eV/atom（若对齐开启，则是相对能量零点附近）
- B0：eV/Å^3（并换算为 GPa）
...
def main():
    ...
    ap.add_argument("--no-emin-align", action="store_true",
                    help="关闭能量零点对齐（默认会对每个方法做 ΔE = E - Emin）")
    args = ap.parse_args()
    ...
    valid_curve_count = 0
    do_align = (not args.no_emin_align)

    for fp in xyz_files:
        method = fp.stem

        try:
            V, E_raw = read_xyz_or_extxyz_VE(fp)
        except Exception as e:
            print(f"[跳过] {fp.name}: {e}")
            continue

        # ====== 新增：每个方法做最低点对齐（去掉常数偏置）======
        Emin_raw = float(np.min(E_raw))
        E = (E_raw - Emin_raw) if do_align else E_raw
        ...
        if not args.no_points:
            label_data = f"{method} (data)"
            if do_align:
                label_data += " (E-Emin)"
            plt.scatter(V, E, marker=mk, label=label_data)
        ...
        if not args.only_points:
            ...
            label_fit = f"{method} (BM3)"
            if do_align:
                label_fit += " (E-Emin)"
            plt.plot(V_fit, E_fit, label=label_fit)
    ...
    plt.ylabel("Energy/Atom (eV)" if not do_align else r"$\Delta E$/Atom (eV)")
