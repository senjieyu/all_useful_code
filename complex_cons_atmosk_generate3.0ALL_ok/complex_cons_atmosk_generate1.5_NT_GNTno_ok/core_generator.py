    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        if mode in ['GNT', 'NT']:
            ...
            for _ in range(ngrains):
                ...
                if mode == 'GNT' and getattr(cfg, 'GNT_GRAIN_GRADIENT', False):
                    # Gradient density: High density at Top (Y=H), Low density at Bottom (Y=0)
                    # Use y = H * sqrt(u) where u in [0,1]
                    # This puts more points near H (Top), creating smaller grains there.
                    u = random.random()
                    y = box[1] * math.sqrt(u)
                else:
...
def run_generation(cfg):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if mode in ['GNT', 'NT']:
            # --- New Logic for GNT/NT ---
            if mode == 'NT':
                spacings = [getattr(cfg, 'NT_SPACING', 5)]
            else:
                spacings = getattr(cfg, 'GNT_SPACINGS', [10, 6, 2])
                
            twin_h = getattr(cfg, 'GNT_TWIN_THICKNESS', 1)
            if mode == 'NT':
                twin_h = getattr(cfg, 'NT_TWIN_THICKNESS', 1)

            # 1. Generate Poly.txt (Nodes)
            poly_txt = os.path.join(tmpdir, "poly.txt")
            write_poly_txt(poly_txt, cfg)
            
            # DEBUG: Print poly.txt content
            print(f"\n[DEBUG] Content of {poly_txt}:")
            with open(poly_txt, 'r') as f:
                print(f.read())
            print("[DEBUG] End of poly.txt\n")
            ...
            for i, sp in enumerate(spacings):
                ...
                # Define Interval: Bottom is 0. 
                # Block 0 is Bottom (0 to H/N) ?? 
                # User config: GNT_SPACINGS = [Bottom, ..., Top]
                # So Block 0 corresponds to Y in [0, block_h]
                ymin = i * block_h
                ymax = (i + 1) * block_h
            ...
            print(f"\n[INFO] Final GNT/NT structure atoms: {n_final}")
            
        else:
            # --- Original Logic for Random/Equal/Ratio ---
            # 1. 生成多层孪晶种子
