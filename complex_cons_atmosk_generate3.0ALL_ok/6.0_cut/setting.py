...
class Config:
    ...
    # 增大至 10 以确保旋转覆盖，减少晶界真空空洞
    # 确保种子尺寸大于 Box 尺寸，特别是 Z 方向
    # Box 是 40x40x40. Cu Unit Cell ~ 3.6A.
    # X direction: [11-2] ~ 8.85A. 6 * 8.85 = 53A > 40A. OK.
    # Z direction: [-110] ~ 5.11A. 6 * 5.11 = 30.6A < 40A. BAD.
    # Need Z > 8 units. Set to 10.
    SEED_DUP_X = 15
    SEED_DUP_Z = 10
    ...
    GNT_ZONES = [
        ...
        #     # "max_layers": 5,
        #     # "layer_count": 4,
        #     "random_orient_X": True,  # 打开表示绕着X轴随机旋转晶粒取向
        #     "random_orient_Y": True,
        #     "random_orient_Z": True,
        #     "enable_twin": False
        # },
        ...
        #     "mode": "equal",
        #     "equal_thickness": 1,
        #     "random_orient_X": False,
        #     "random_orient_Y": True,
        #     "random_orient_Z": False,
        #     "enable_twin": True
        # },
    ...
    # 最终多晶盒子的尺寸 (Angstrom)
    # 用户示例: 600 400 200
    # 为了演示快速运行，这里暂时设小一点，正式跑可改回 [200, 600, 200]
    POLY_BOX = [40, 40, 40]
    ...
    # [原子合并控制]
    # 该参数控制“合并”的判定范围。
    # 逻辑说明: 程序会查找距离小于此值的原子对，并计算它们的【平均中心位置】生成一个新原子 (2合1)。
    #           (注意：这不是简单的删除，而是融合，能更准确地保留晶界形状)
    #
    # [数值建议]:
    # - 1.2 (当前值): 推荐。适用于 Cu (最近邻~2.55A)。小于 1.2A 的重叠会被合并。
    # - 调大 (如 1.5): 合并更激进，会让晶界更稀疏。
    # - 调小 (如 0.8): 合并更保守，仅处理极度重叠的原子。
    MERGE_DOUBLES_DIST = 1.5
