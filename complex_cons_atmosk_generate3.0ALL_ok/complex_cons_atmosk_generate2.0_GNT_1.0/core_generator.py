...
def write_poly_txt(path, cfg, box_dims=None, n_grains=None):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        if mode == 'NT':
            ...
            # Check for random orientation switch (default True if not set)
            is_random_orient = getattr(cfg, 'NT_GRAIN_ORIENT_RANDOM', True)
            print(f"[INFO] Grain Orientation Random: {is_random_orient}")
...
class ZoneConfig:
    ...
    def __init__(self, parent_cfg, zone_dict):
        ...
        self.NT_GRAIN_ORIENT_RANDOM = zone_dict.get('random_orient', True)
...
def run_generation(cfg):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if mode == 'GNT':
            # --- New Logic for GNT (Stacking Zones) ---
            print("[INFO] Running GNT Mode (Zone Stacking)...")
            ...
            # We must generate blocks sequentially and stack them.
            current_y = 0.0
            
            for i, z_conf in enumerate(zones):
                # 1. Calculate Zone Height
                h_ratio = z_conf['ratio']
                zone_h = total_h * (h_ratio / total_ratio)
                
                print(f"\n[STEP] Generating Zone {i+1}/{len(zones)} (H={zone_h:.2f})...")
                print(f"       Config: {z_conf}")
                
                # 2. Prepare Zone Config
                # Create a temporary config object for this zone
                z_cfg = ZoneConfig(cfg, z_conf)
                
                # 3. Build Seed (Layered or Simple based on mode)
                # We reuse build_layered_seed as it handles all modes (random/equal/ratio/all_Equal)
                # Note: build_layered_seed uses z_cfg.TWIN_MODE
                # We pass the mode from z_conf['mode'] as override
                seed_mode = z_conf.get('mode', 'equal')
                seed_i = build_layered_seed(atomsk, z_cfg, tmpdir, timeout_s, out_dir, mode_override=seed_mode)
                
                # 4. Build Polycrystal for this Zone
                # We need a specific poly.txt for this zone size
                # The box dimensions for this zone: [X, zone_h, Z]
                zone_box = [cfg.POLY_BOX[0], zone_h, cfg.POLY_BOX[2]]
                poly_txt_name = f"poly_zone_{i}.txt"
                poly_txt_path = os.path.join(tmpdir, poly_txt_name)
                
                # We use 'NT' logic (via z_cfg.TWIN_MODE='NT') to ensure grains rotate around Y axis
                # This keeps twin planes parallel (horizontal)
                write_poly_txt(poly_txt_path, z_cfg, box_dims=zone_box, n_grains=z_conf['n_grains'])
                
                block_name = f"block_{i}.lmp"
                block_path = os.path.join(tmpdir, block_name)
                
                # Run Polycrystal
                # Note: Atomsk polycrystal uses the box defined in poly.txt
                run_cmd_stream(
                    [atomsk, "--polycrystal", seed_i, poly_txt_path, block_path, "-wrap", "-overwrite"],
                    cwd=tmpdir, timeout_s=timeout_s, log_path=None
                )
                
                block_files.append(block_path)
                
            # 5. Merge Blocks (Stacking)
            print(f"\n[INFO] Stacking {len(block_files)} zones...")
            
            # Use absolute path in tmpdir for Atomsk output to avoid CWD/relative path issues
            poly_tmp = os.path.join(tmpdir, "poly.lmp")
            
            # atomsk --merge Y N file1 file2 ... out
            merge_cmd = [atomsk, "--merge", "Y", str(len(block_files))] + block_files + [poly_tmp, "-overwrite"]
            run_cmd_stream(merge_cmd, cwd=tmpdir, timeout_s=timeout_s, log_path=os.path.join(out_dir, "log_step5_stack.txt"))
            
            # Copy to final destination
            poly_out = os.path.join(out_dir, "poly.lmp")
            shutil.copy(poly_tmp, poly_out)
